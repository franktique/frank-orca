...
describe('hasUsageProviderSettings', () => {
  ...
  it('treats explicit non-managed provider settings as configured usage providers', () => {
    ...
    expect(hasUsageProviderSettings(usageSettings({ copilotUsageConfigured: true }))).toBe(true)
  })
...
describe('hasUsageProviderSettingsForProvider', () => {
  ...
  it('treats copilotUsageConfigured as the durable signal for Copilot', () => {
    // Why: Orca keeps no Copilot credential; the CLI's own cache file on disk
    // is the only durable signal, relayed by main as this flag.
    expect(
      hasUsageProviderSettingsForProvider(
        'copilot',
        usageSettings({ copilotUsageConfigured: true })
      )
    ).toBe(true)
      ...
      isUsageEmptyState(
        {
          ...
          copilot: provider('fetching', { provider: 'copilot' })
        },
        usageSettings()
      )
    ).toBe(false)
